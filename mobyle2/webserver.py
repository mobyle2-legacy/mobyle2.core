...
from mobyle2 import utils
from pyramid.threadlocal import get_current_registry
from pyramid.exceptions import Forbidden

from sqlalchemy import engine_from_config

from mobyle2.models import initialize_sql, DBSession
from mobyle2.config import dn
from mobyle2.auth import AuthTktAuthenticationPolicy, ACLAuthorizationPolicy
from mobyle2.models.registry import get_registry_key, set_registry_key

from mobyle2.interfaces import IMobyle2View
...
mobyle2_templates = resource_filename('mobyle2', 'templates/deform')
...
def includeme(config, debug=False):
    ...
    config.add_view('mobyle2.views.root.RedirectAfterLogin', route_name='redirect_after_login', permission = P['global_view'])
    # apex overrides
    render_template = 'mobyle2:templates/apex_template.pt'
    config.add_view('mobyle2.views.apexviews.login', route_name='apex_login', renderer=render_template, permission = P['global_view'])
    config.add_view('mobyle2.views.apexviews.register', route_name='apex_register', renderer=render_template, permission = P['global_view'])
    config.add_view('mobyle2.views.apexviews.forgot', route_name='apex_forgot', renderer=render_template, permission = P['global_view'])
    config.add_view('mobyle2.views.apexviews.reset', route_name='apex_reset', renderer=render_template, permission = P['global_view'])
    config.add_view('mobyle2.views.apexviews.activate', route_name='apex_activate', renderer=render_template, permission = P['global_view'])
    config.add_view('mobyle2.views.apexviews.useradd', route_name='apex_useradd', renderer=render_template, permission = P['global_admin'])
    config.add_view('mobyle2.views.auth.forbidden', context=Forbidden)
    config.end()
    config.commit()
    from mobyle2.models.registry import set_registry_key
    from mobyle2.models.project import create_public_workspace
    from mobyle2.models.server import create_local_server
    from mobyle2.models.project import projects_dir, PROJECTS_DIR
